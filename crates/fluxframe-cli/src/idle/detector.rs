...
use fluxframe_gst::ClientUsage;
use fluxframe_gst::output::OutputSink;
use fluxframe_gst::v4l2_events::ConsumerWatch;
use tracing::{debug, info, warn};

use super::resync::{DeviceProbe, NodeProbe, ResyncSchedule, ResyncState, resync_tick};
...
pub(super) fn shutdown_bounded_timeout_ms() -> i32 {
...
/// readable.
///
/// Visible to the rest of `idle` because [`super::resync`] runs on this
/// same thread and against this same context — the two modules split
/// the detector's work, not its state.
pub(super) struct DetectorContext<'a> {
    pub(super) proc_root: &'a Path,
    pub(super) my_pid: u32,
    running: &'a AtomicBool,
    status: &'a AtomicU8,
    pub(super) stopped: &'a AtomicBool,
    restat_interval_polls: u32,
    pub(super) device_basename: &'a OsStr,
...
///
/// `last_status` is a plain `&mut`, which is the reason both this and
/// [`super::resync`] must stay on the detector thread — see
/// [`super::resync::resync_tick`].
pub(super) struct DetectorState<'a> {
    log_once: &'a mut LogOnce,
    pub(super) last_status: &'a mut Option<ConsumerStatus>,
...
pub(super) fn read_comm(proc_root: &Path, pid: u32) -> Option<String> {
...
pub(super) fn count_external_consumers(
    proc_root: &Path,
...
// `pub(super)` so [`super::resync`]'s tests can reuse the `/proc`
// fixture and the context builder rather than growing a second copy of
// each.
#[cfg(test)]
pub(super) mod tests {
    use super::*;
    ...
    use super::super::resync::fixtures::{FakeProbe, probe_ok};
    ...
    pub(in crate::idle) fn with_ctx<R>(
        proc_root: &Path,
        counters: &Counters,
    ...
    pub(in crate::idle) fn make_proc_entry(proc_root: &Path, pid: u32, fd: u32, target: &Path) {
